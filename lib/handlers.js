...
handlers.index = (data,callback) => {
  ...
  if (data.method == 'get') {
    ...
    let templateData = {
      ...
      'head.description': 'We offer free, simple uptime monitoring for HTTP/HTTPS sites of all kind, when your site goes down we\'ll let you know.',
      'body.class': 'index'
    };
    ...
    helpers.getTemplate('index',templateData,(err,str) => {
      if (!err && str) {
        // Add the uinversal template header and footer
...
handlers.accountDeleted = (data, callback) => {
  ...
  if (data.method == 'get') {
    ...
    let templateData = {
      ...
      'header.description': 'Your account has been deleted',
...
handlers.favicon = (data,callback) => {
  // Reject any reqest that isn't a GET
  if (data.method == 'get') {
...
// Container for the users submethids
handlers._users ={};

// Users - post
// Requier data: firstname, lastname, phone, password, tosAgreement
// Optional data: none
  ...
  if(phone) {
    ...
    handlers._tokens.verifyToken(token,phone,(tokenIsValid) => {
      if (tokenIsValid) {
        ...
        _data.read('users', phone, (err, data) => {
          if (!err && data) {
            // Remove the hashed password from the user oobject before returning the data
        ...
        if (tokenIsValid) {
          ...
          _data.read('users', phone, (err, userData) => {
            if (!err && userData) {
              ...
              callback(400, { 'Error': 'The specified user does\'n exist' });
            }
          });
        } else {
          callback(403, { 'Error': 'Missing required token in header, or token is invalid' });
        ...
        _data.read('users', phone, (err, userData) => {
          if (!err && userData) {
            _data.delete('users', phone, (err) => {
              if (!err) {
                // Delet each of the checks associated with the user
                    ...
                    _data.delete('checks', checkId, (err) => {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, {'Error': 'Errors encountered while attempting to delete all of the user\'s checks. All checkes may not have been deleted from the system successfully'});
                        }
                      }
                    });
                  });
...
handlers._tokens.post = (data, callback) => {
  ...
  if (phone && password) {
    ...
    _data.read('users',phone,(err, userData) => {
      if (!err && userData) {
        // Hash the sent password and compare in with the password stored in the users data
        let hashedPassword = helpers.hash(password);
        if (hashedPassword == userData.hashedPassword) {
          // if valid create a new token with a random name. Set expiration date 1hr in the futuer
          let tokenId = helpers.createRandomString(20);
          let expiers = Date.now() + 1000 * 60 * 60;
          let tokenObject = {
            'phone': phone,
            'id': tokenId,
            'expires': expiers
    ...
    _data.read('tokens',id,(err,tokenData) => {
      if (!err && tokenData) {
        ...
        if (tokenData.expires > Date.now()) {
          // Set the expation an hour from now
          ...
          callback(400, { 'Error': 'The token has already expired and can not b extended' });
        }
      } else {
...
// Verify if a given token id id currently valid for a given user
...
// Container for all the cjecks methods
handlers._checks = {};
  ...
  if (id) {
    ...
    _data.read('checks', id, (err, checkData) => {
      if (!err && checkData) {
        // Get the token from the headers
        let token = typeof (data.headers.token) == 'string' ? data.headers.token : false;
        // verify that the given token is belongs to the users who created the check
              ...
              if (tokenIsValid) {
                ...
                _data.read('users', checkData.userPhone, (err, userData) => {
                  if (!err && userData) {
                    ...
                    if (checkPosition > -1) {
                      ...
                      _data.update('users', checkData.userPhone, userData, (err) => {
                        if (!err) {
                          ...
                          callback(500, { 'Error': 'Could not updat user' });
                        }
                      });
                    } else {
                    ...
                    callback(500, { 'Error': 'Could not find the the user who created the check, so could not remove the check from the list of checks in the user object' })
                  }
                });
          } else {
